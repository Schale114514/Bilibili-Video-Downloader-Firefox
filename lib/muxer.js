...
(function (global) {
  ...
  /* 解析 moov 里的 mvex/trex：分片流的默认样本时长/大小/标志
   * B站视频流常在 trun 中不写样本时长，由 demuxer 用 trex 默认值兜底 */
  function parseTrexDefaults(u8, moov) {
    const map = new Map();
    const mvex = findChild(u8, moov, 'mvex');
    if (!mvex) return map;
    for (const b of childBoxes(u8, mvex)) {
      if (b.type !== 'trex') continue;
      const dv = new DataView(u8.buffer);
      const o = b.start + b.header; // version+flags(4) 之后
      map.set(dv.getUint32(o + 4), {
        duration: dv.getUint32(o + 12),
        size: dv.getUint32(o + 16),
        flags: dv.getUint32(o + 20)
      });
    }
    return map;
  }

  function parseMoof(u8, moof, top, moofIndex, trexDefaults) {
    const dv = new DataView(u8.buffer);
    ...
    for (const traf of trafs) {
      ...
      const tDef = trexDefaults ? trexDefaults.get(tId) : null;
      ...
      else if (tDef) defaultDuration = tDef.duration;
      if (tfFlags & 0x000010) { defaultSize = dv.getUint32(o); o += 4; }
      else if (tDef) defaultSize = tDef.size;
      if (tfFlags & 0x000020) { defaultFlags = dv.getUint32(o); o += 4; }
      else if (tDef) defaultFlags = tDef.flags;
      let baseDts = 0;
      if (tfdt) {
  ...
  function parseFmp4(input) {
    ...
    const trexDefaults = parseTrexDefaults(u8, moov);
    for (let i = 0; i < top.length; i++) {
      const b = top[i];
      if (b.type !== 'moof') continue;
      const frag = parseMoof(u8, b, top, i, trexDefaults);
      if (frag.trackId) {
  ...
  /* 注意：fullBox 不会自动补写 version+flags，调用方的 body 必须自己以
   * u8(version) + u24(flags) 开头（这是 dref 盒曾漏写 4 字节、导致 Windows
   * Media Foundation 拒播的教训，见 bDinf 与 muxer.test.mjs 的 dinf/dref 断言） */
  function fullBox(type, body) {
    return makeBox(type, body);
  }

  /* 输出文件使用标准 MP4 品牌：
   * 源流 ftyp 常带 dash/msix/dsms 等分片流媒体品牌，Windows 自带播放器会因此拒播；
   * 改写为通用品牌后兼容性最好（AVC 内容追加 avc1 品牌） */
  function bFtyp(hasAvc) {
    const b = new Buf();
    b.u32(type4('isom')); // major_brand
    b.u32(0x200);         // minor_version
    b.u32(type4('isom'));
    b.u32(type4('iso2'));
    b.u32(type4('mp41'));
    if (hasAvc) b.u32(type4('avc1'));
    return makeBox('ftyp', b.result());
  }

  function bMvhd(duration) {    const b = new Buf();
    b.u8(0); b.u24(0);
  ...
  function bDinf() {
    const urlBox = makeBox('url ', (function () {
      const b = new Buf(); b.u8(0); b.u24(1); return b.result();   // flags=1: 数据在同一文件内
    })());
    const dref = fullBox('dref', (function () {
      const b = new Buf();
      b.u32(0);                 // version + flags（必须为 0）
      b.u32(1);                 // entry_count
      b.raw(urlBox);
      return b.result();
    })());
  ...
  function bCtts(samples) {
    ...
    // 出现负合成偏移（B 帧）时必须用 version 1（v0 偏移为无符号，部分播放器会读错）
    const needV1 = runs.some(function (r) { return r.o < 0; });
    const b = new Buf();
    b.u8(needV1 ? 1 : 0); b.u24(0);
    b.u32(runs.length);
  ...
  function bStco(offsets, useCo64) {
    if (useCo64) {
      // 大文件（>4GB）：64 位 chunk 偏移表
      const b = new Buf();
      b.u32(0);
      b.u32(offsets.length);
      for (const o of offsets) {
        b.u32(Math.floor(o / 0x100000000));
        b.u32(o >>> 0);
      }
      return fullBox('co64', b.result());
    }
    const b = new Buf();
    b.u32(0);
    b.u32(offsets.length);
  ...
  /* 编辑列表 edts/elst：
   * ffmpeg 等标准封装器都会写：用 media_time = 首样本合成偏移(cts) 裁掉 B 帧重排的前导偏移，
   * 让首帧从 0 时刻开始显示。缺少它会令 Windows Media Foundation 等严格播放器拒播。
   * trimTicks: 首样本 cts（轨道时基）；segMovieTs: 裁剪后时长（影片时基 1000） */
  function bEdts(trimTicks, segMovieTs, delayMovieTs) {
    const elst = (function () {
      const b = new Buf();
      b.u8(0); b.u24(0);
      const count = (delayMovieTs > 0 ? 1 : 0) + 1;
      b.u32(count);                       // entry_count
      if (delayMovieTs > 0) {
        b.u32(delayMovieTs);              // 空编辑：延迟该轨开始
        b.i32(-1);                        // media_time = -1
        b.u16(1); b.u16(0);
      }
      b.u32(segMovieTs);                  // segment_duration (movie timescale)
      b.i32(trimTicks);                   // media_time = 首样本合成偏移
  ...
  function bStbl(samples, sampleEntry, isVideo, useCo64) {
    ...
    parts.push(bStsc(), bStsz(samples), bStco(samples.map(function (s) { return s.offset; }), useCo64));
    return makeBox('stbl', concat(parts));
  }

  function bMinf(track, useCo64) {
    const parts = [];
    if (track.handler === 'vide') parts.push(bVmhd());
    else if (track.handler === 'soun') parts.push(bSmhd());
    parts.push(bDinf());
    parts.push(bStbl(track.samples, track.sampleEntry, track.handler === 'vide', useCo64));
    return makeBox('minf', concat(parts));
  }

  function bMdia(track, useCo64) {
    const parts = [bMdhd(track.timescale, track.duration), track.hdlr, bMinf(track, useCo64)];
    return makeBox('mdia', concat(parts));
  }

  function bTrak(track, trackId, offsets, useCo64) {
    ...
    parts.push(bEdts(track.trimTicks, track.editDuration, track.delay));  // 始终写编辑列表（裁掉 B 帧前导偏移）
    parts.push(bMdia(track, useCo64));
    return makeBox('trak', concat(parts));
  }

  function bMoov(tracks, offsetsList, useCo64) {
    const parts = [bMvhd(Math.max.apply(null, tracks.map(function (t) { return t.tkhdDuration; })))];
    tracks.forEach(function (t, i) { parts.push(bTrak(t, i + 1, offsetsList[i], useCo64)); });
  ...
  function prepareTrack(parsed, sourceU8) {
    ...
    // B 帧重排会导致首样本 cts > 0：用编辑列表 media_time 裁掉这段前导偏移
    const trimTicks = Math.max(0, samples[0].cts);
    const trimmed = Math.max(0, duration - trimTicks);
    return {
      ...
      duration: duration,
      trimTicks: trimTicks,
      editDuration: Math.round(trimmed / ts * 1000),   // 裁剪后的时长（影片时基）
      delay: 0,
      tkhdDuration: Math.round(trimmed / ts * 1000)
    };
  }

  /* ---------------- 对外主函数 ---------------- */

  /* mergeToMp4(videoArrayBuffer, audioArrayBuffer?, opts?)
   * opts.moovLast : 把 moov 写在 mdat 之后（与 ffmpeg 默认输出一致）
   * opts.freeBox  : 在 ftyp 后插入 8 字节 free 盒（对齐 ffmpeg 输出布局） */
  function mergeToMp4(videoInput, audioInput, opts) {
    const moovLast = !!(opts && opts.moovLast);
    const freeBox = !!(opts && opts.freeBox);
    ...
    // 两条轨道都用编辑列表把首帧对齐到 0 时刻（media_time = 首样本合成偏移），
    // 因此这里不再需要“空编辑延迟”式的对齐
    ...
    const totalSampleBytes = tracks.reduce(function (acc, t) {
      return acc + t.samples.reduce(function (x, s) { return x + s.size; }, 0);
    }, 0);

    const ftyp = bFtyp(vp.sampleEntry && vp.sampleEntry.length >= 8 &&
      String.fromCharCode(vp.sampleEntry[4], vp.sampleEntry[5], vp.sampleEntry[6], vp.sampleEntry[7]).indexOf('avc') === 0);
    const pre = freeBox ? concat([ftyp, makeBox('free', new Uint8Array(0))]) : ftyp;

    // 4K 长片合并后可能超过 4GB：需要 64 位 mdat 大小(largesize) 与 64 位偏移表(co64)
    const useLargesize = (totalSampleBytes + 8) > 0xffffffff;
    const mdatHeaderSize = useLargesize ? 16 : 8;

    const dummy = tracks.map(function (t) { return t.samples.map(function () { return 0; }); });
    let useCo64 = false;
    let moovSize = 0;   // moov 在 mdat 之前时才影响样本偏移
    if (moovLast) {
      useCo64 = (pre.length + mdatHeaderSize + totalSampleBytes) > 0xffffffff;
    } else {
      let moov1 = bMoov(tracks, dummy, false);
      if (pre.length + moov1.length + mdatHeaderSize + totalSampleBytes > 0xffffffff) {
        useCo64 = true;
        moov1 = bMoov(tracks, dummy, true);
      }
      moovSize = moov1.length;
    }

    // 布局：交错存放样本，计算每个样本在输出文件中的绝对偏移
    let cursor = pre.length + moovSize + mdatHeaderSize;
    ...
    const mdatSize = totalSampleBytes;

    const moov2 = bMoov(tracks, offsets, useCo64);
    if (!moovLast && moov2.length !== moovSize) throw new Error('muxer: 内部布局不一致');

    let mh;
    if (useLargesize) {
      mh = new Uint8Array(16);
      const mhdv = new DataView(mh.buffer);
      mhdv.setUint32(0, 1);                       // largesize 标志
      mhdv.setUint32(4, type4('mdat'));
      const total = mdatSize + 8;
      mhdv.setUint32(8, Math.floor(total / 0x100000000));
      mhdv.setUint32(12, total >>> 0);
    } else {
      mh = new Uint8Array(8);
