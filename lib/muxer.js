...
(function (global) {
  ...
  function mergeToMp4(videoInput, audioInput) {
    ...
    const totalSampleBytes = tracks.reduce(function (a, t) {
      return a + t.samples.reduce(function (x, s) { return x + s.size; }, 0);
    }, 0);
    ...
    // 4K 长片合并后可能超过 4GB：需要 64 位偏移表(co64) 与 64 位 mdat 大小(largesize)
    let useCo64 = false;
    let moov1 = bMoov(tracks, dummy, false);
    let headerSize = ftyp.length + moov1.length;
    if (headerSize + 8 + totalSampleBytes > 0xffffffff) {
      useCo64 = true;
      moov1 = bMoov(tracks, dummy, true);
      headerSize = ftyp.length + moov1.length;
    }
    ...
    const useLargesize = (mdatSize + 8) > 0xffffffff;
